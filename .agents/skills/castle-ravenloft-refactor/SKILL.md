---
name: castle-ravenloft-refactor
description: Guides refactor decisions, architectural cleanup, performance work, and behavior-preserving code changes in the Castle Ravenloft React Vite Three.js Zustand project.
---
...
User instructions in the current session

Manually verified Castle Ravenloft card text and project source-of-truth files

Current engine behavior that has already been intentionally implemented

Existing UI behavior

Documentation files such as agents.md
...
Identify whether the change is structural, behavioral, or both.

If behavioral impact is possible, explicitly state it before editing.

Prefer extraction and simplification before semantic rewrites.

Do not delete suspicious files unless usage has been verified.
...
Run the real available validation commands for the repo

Prefer npm run build when it includes TypeScript compilation

Use npx tsc --noEmit when a direct type check is needed and available

Run npx tsx runTests.ts when the task affects gameplay logic or shared engine behavior

Report exactly what changed, why it is safe, and any remaining risks
