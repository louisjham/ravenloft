...
export const createPowerSlice: StateCreator<GameStore, [], [], PowerSlice> = (set, get) => ({
  ...
  confirmHeroSelection: (heroId: string) => {
      ...
      const confirmResult = PowerSelectionSystem.confirmSelection(selection, constraints);

      if ('error' in confirmResult) {
        return; // Selection is not valid yet
      }

      // Add selected power IDs to the hero
      const selectedPowerIds = confirmResult.selectedPowerIds;
  ...
  autoSelectPowers: (heroId: string) => {
      ...
      const updatedSelection = PowerSelectionSystem.autoSelectPowers(hero.heroClass, heroId, constraints);
