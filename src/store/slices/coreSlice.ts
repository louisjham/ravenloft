...
import { GameState, Entity, Tile, Card, GameSettings } from '../../game/types';
...
export const createCoreSlice: StateCreator<GameStore, [], [], CoreSlice> = (set, get) => ({
  ...
  startNewGame: (scenarioId: string, heroIds: string[]) => {
    ...
    const selectedHeroes = heroIds.map(id => allHeroes.find(h => h.id === id)).filter(Boolean) as import('../../game/types').Hero[];
    ...
    const initialState: GameState = {
      ...
      ...(() => {
        ...
        const hasTimeTrack = rules.some((r: { id?: string; description?: string }) =>
          r.id?.includes('sun_track') || r.id?.includes('daylight') || r.id?.includes('collapse') ||
          r.description?.toLowerCase().includes('time track') ||
          r.description?.toLowerCase().includes('sunset') ||
          r.description?.toLowerCase().includes('advance the time')
        );
        if (hasTimeTrack) {
        ...
        // Strahd awakening
        if (rules.some((r: { id?: string }) => r.id?.includes('strahd_awakens'))) {
    ...
    if (currentEntity) {
      ...
      if (poisonDamage > 0) {
        ...
        if (currentEntity.type === 'hero') {
          updatedHeroes = updatedHeroes.map(h => h.id === currentEntity.id ? updated as import('../../game/types').Hero : h);
        }
      }
      const processedEntity = ConditionSystem.processTurnEnd(currentEntity);
      if (currentEntity.type === 'hero') {
        updatedHeroes = updatedHeroes.map(h => h.id === currentEntity.id ? processedEntity as import('../../game/types').Hero : h);
      } else {
        updatedMonsters = updatedMonsters.map(m => m.id === currentEntity.id ? processedEntity as import('../../game/types').Monster : m);
      }
    }
    ...
    const updatedHeroes = state.heroes.map(hero => {
      if (hero.id === heroId) {
        return {
          ...
          maxHp: hero.maxHp + 2,
          ac: hero.ac + 1,
          attackBonus: (hero as any).attackBonus ? (hero as any).attackBonus + 1 : 1,
