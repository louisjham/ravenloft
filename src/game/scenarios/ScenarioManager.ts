...
export class ScenarioManager {
  ...
  public static processPostExplore(gameState: GameState, placedTile: Tile): GameState {
    ...
    const hasArcaneRule = rules.some((r: { id?: string }) => r.id === 'arcane_circle_spawns');
    if (hasArcaneRule && placedTile.id === 'named_arcane_circle' && placedTile.isRevealed) {
      state = ScenarioManager.handleArcaneCircleReveal(state);
    }

    // Check for Chapel reveal (Adventure 2)
    if (placedTile.id === 'named_chapel' && placedTile.isRevealed) {
      state = {
        ...state,
        chapelRevealed: true,
        log: [
  ...
  private static triggerStrahdAwakening(state: GameState): GameState {
    const strahdMonster = DataLoader.getInstance().getMonsterById('villain_strahd');
