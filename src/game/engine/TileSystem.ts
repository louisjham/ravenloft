import { Tile, Position, TileConnection, Direction, Rotation, GameState, ExplorationPoint, EdgeConflict, ValidationResult, EdgeDirection, Monster } from '../types';
      ...
      tile.connections.forEach(conn => {
        if (conn.isOpen && !conn.connectedTileId) {
          ...
          if (TileSystem.canPlaceTile(tiles, targetCoords.x, targetCoords.z)) {
            const TILE_SIZE = 4;
            const CENTER_OFFSET = 1.5; // (0+1+2+3)/4
