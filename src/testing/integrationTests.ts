...
export const runFullGameLoopTest = async () => {
  ...
  try {
    ...
    console.log('  Test 5: autoSelectPowers returns exactly 4 confirmed ids...');
    {
      ...
      // Since database only has 1 at-will card for paladin, autoSelect will select 1 at-will and distribute the rest
      if (atWillCount !== 1 || (dailyCount !== 1 && dailyCount !== 2) || (utilityCount !== 1 && utilityCount !== 2)) {
    ...
    console.log('Testing Adventure 1: Escape the Tomb...');
    {
      ...
      if (!state.timeTrack || state.timeTrack.current !== 1 || state.timeTrack.max !== 6) {
    ...
    console.log('Testing Adventure 2: Find the Icon of Ravenloft...');
    {
      ...
      // Check pre-placement: Start Tile and two corridor tiles adjacent to it ((0, 1) and (0, -1))
      if (state.tiles.length !== 3) {
        throw new Error(`Adventure 2: expected 3 initial tiles, got ${state.tiles.length}`);
      }

      const hasStartTile = state.tiles.some(t => t.x === 0 && t.z === 0 && t.id === 'start-tile');
      const hasSouthCorridor = state.tiles.some(t => t.x === 0 && t.z === 1 && t.terrainType === 'corridor');
      const hasNorthCorridor = state.tiles.some(t => t.x === 0 && t.z === -1 && t.terrainType === 'corridor');

      if (!hasStartTile || !hasSouthCorridor || !hasNorthCorridor) {
        throw new Error('Adventure 2: initial tiles or coordinates incorrect');
      }
      ...
      // Should spawn N monsters (where N is the number of active heroes = 1)
      if (stateAfterChapel.monsters.length !== 1) {
        throw new Error(`Adventure 2: expected 1 spawned monster on Chapel, got ${stateAfterChapel.monsters.length}`);
      }

      // Should place the Icon of Ravenloft token at its center
      const hasIconToken = stateAfterChapel.tokens?.some(t => t.id === 'token_item_icon_of_ravenloft' && t.tileId.startsWith('named_chapel') && t.position.sqX === 2 && t.position.sqZ === 2);
      if (!hasIconToken) {
        throw new Error('Adventure 2: Icon of Ravenloft token missing or placed incorrectly');
      }

      // Target IDs for objective clear_chapel_monsters should contain the spawned monster's ID
      const clearObj = stateAfterChapel.activeScenario.objectives.find(obj => obj.type === 'clear_chapel_monsters');
      if (!clearObj || !clearObj.targetIds || clearObj.targetIds.length !== 1 || clearObj.targetIds[0] !== stateAfterChapel.monsters[0].id) {
        throw new Error(`Adventure 2: clear_chapel_monsters objective targetIds incorrect: ${JSON.stringify(clearObj)}`);
      }

      // Test permanent threat: draw Encounter card at start of Villain Phase
      console.log('  Testing permanent threat Encounter card drawing...');
      if (stateAfterChapel.encounterDeck.length === 0) {
        stateAfterChapel.encounterDeck = ['mock_encounter'];
      }
      
      const stateBeforeVillain = {
        ...stateAfterChapel,
        cardResolution: undefined
      };
      
      const stateAfterVillain = await executeVillainPhase(stateBeforeVillain);
      if (!stateAfterVillain.cardResolution || stateAfterVillain.cardResolution.phase !== 'drawing' || stateAfterVillain.cardResolution.cardType !== 'encounter') {
        throw new Error(`Adventure 2: expected permanent threat Encounter card drawing, got: ${JSON.stringify(stateAfterVillain.cardResolution)}`);
      }

      // Test picking up the Icon token and completing the victory objective
      console.log('  Testing Icon token pickup and objective completion...');
      
      // Move hero to the Chapel tile and invoke endTurn
      const hero = stateAfterChapel.heroes[0];
      const chapelTileObj = stateAfterChapel.tiles.find(t => t.id.startsWith('named_chapel'))!;
      
      const stateWithHeroAtChapel: GameState = {
        ...stateAfterChapel,
        heroes: stateAfterChapel.heroes.map(h => ({
          ...h,
          position: { x: chapelTileObj.x, z: chapelTileObj.z, sqX: 2, sqZ: 2 }
        }))
      };

      // Run endTurn manually via the store
      useGameStore.setState({ gameState: stateWithHeroAtChapel });
      store.endTurn();

      const endTurnState = useGameStore.getState().gameState;
      if (!endTurnState) throw new Error('Adventure 2: endTurnState not set');

      // Token should be removed
      const hasIconTokenAfter = (endTurnState as GameState).tokens?.some(t => t.id === 'token_item_icon_of_ravenloft');
      if (hasIconTokenAfter) {
        throw new Error('Adventure 2: Icon token should be removed after endTurn');
      }

      // Hero should possess the treasure_icon_ravenloft card
      const updatedHero = (endTurnState as GameState).heroes[0];
      if (!updatedHero.items.includes('treasure_icon_ravenloft')) {
        throw new Error('Adventure 2: hero should possess treasure_icon_ravenloft items list');
      }

      // possess_icon objective should be completed
      const possessObj = (endTurnState as GameState).activeScenario.objectives.find(obj => obj.type === 'possess_icon');
      if (!possessObj || !possessObj.isCompleted) {
        throw new Error('Adventure 2: possess_icon objective should be marked completed');
      }

      // Defeat the monster and verify clear_chapel_monsters completion
      console.log('  Testing clear_chapel_monsters objective completion...');
      
      const stateWithDeadMonster: GameState = {
        ...endTurnState,
        monsters: (endTurnState as GameState).monsters.map(m => ({ ...m, hp: 0, isDefeated: true }))
      };

      // Run checkObjectives manually
      const finalObjectives = ObjectiveTracker.checkObjectives(stateWithDeadMonster);
      const clearChapelObj = finalObjectives.find(obj => obj.type === 'clear_chapel_monsters');
      if (!clearChapelObj || !clearChapelObj.isCompleted) {
        throw new Error('Adventure 2: clear_chapel_monsters should be completed when monster is defeated');
      }
    ...
    console.log('Testing Adventure 3: Klak\'s Infernal Artifact...');
    {
      ...
      // Initialize Adventure 3 with Hero Arjhan
      ...
      // Check deck insertion: Laboratory tile (named_laboratory) should be placed at index 8 in the deck
      ...
      // Test Laboratory tile placement, Klak spawn, and Artifact token placement
      console.log('  Testing Laboratory placement and spawn triggers...');
      
      const mockLabTile: Tile = {
        id: 'named_laboratory',
        name: 'Laboratory',
        x: 1, z: 0,
        terrainType: 'named_room',
      ...
      TileSystem.drawAndPlace = () => ({
        tile: mockLabTile,
        validRotations: [0],
        remainingDeck: state.dungeonDeck.filter(id => id !== 'named_laboratory'),
        exhausted: false
      });

      let stateAfterLab: GameState;
      try {
        stateAfterLab = TileSystem.placeTile(
          state,
          { tileId: 'start-tile', edge: 'east' },
          0
        );
      ...
      if (stateAfterLab.laboratoryRevealed !== true) {
        throw new Error('Adventure 3: laboratoryRevealed should be set to true');
      }

      // Should spawn Klak and Klak's Artifact
      const klak = stateAfterLab.monsters.find(m => m.id.startsWith('monster_klak'));
      const artifact = stateAfterLab.monsters.find(m => m.id.startsWith('monster_klaks_artifact'));

      if (!klak) throw new Error('Adventure 3: Klak monster was not spawned');
      if (!artifact) throw new Error('Adventure 3: Klak\'s Artifact monster was not spawned');

      if (klak.hp !== 6 || klak.ac !== 14 || !klak.isBoss) {
        throw new Error(`Adventure 3: Klak has incorrect stats: HP ${klak.hp}, AC ${klak.ac}, isBoss ${klak.isBoss}`);
      }

      if (artifact.hp !== 5 || artifact.ac !== 15 || artifact.isBoss) {
        throw new Error(`Adventure 3: Klak's Artifact has incorrect stats: HP ${artifact.hp}, AC ${artifact.ac}, isBoss ${artifact.isBoss}`);
      }

      // Should place the Artifact token
      const hasArtifactToken = stateAfterLab.tokens?.some(t => t.id === 'token_adventure_klaks_artifact' && t.tileId.startsWith('named_laboratory'));
      if (!hasArtifactToken) {
        throw new Error('Adventure 3: Klak\'s Artifact token missing or placed incorrectly');
      }

      // Test XP Cancel Penalty Rule
      console.log('  Testing XP cancel penalty rule...');
      const stateWithXP: GameState = {
        ...stateAfterLab,
        experiencePile: ['monster_card_1', 'monster_card_2', 'monster_card_3', 'monster_card_4', 'monster_card_5']
      };
      
      useGameStore.setState({ gameState: stateWithXP });
      
      const beforeHp = stateWithXP.heroes.find(h => h.id === stateWithXP.currentHeroId)?.hp || 0;
      
      store.cancelEncounterCard('enc_vampire_spawn');
      
      const afterCancelState = useGameStore.getState().gameState;
      if (!afterCancelState) throw new Error('Adventure 3: afterCancelState not set');
      
      const afterHp = afterCancelState.heroes.find(h => h.id === afterCancelState.currentHeroId)?.hp || 0;
      if (afterHp !== beforeHp - 1) {
        throw new Error(`Adventure 3: XP cancel rule did not deal 1 damage to active hero. Before: ${beforeHp}, After: ${afterHp}`);
      }

      // Test Discard Treasure for Power Rule
      console.log('  Testing discard treasure for power rule...');
      const heroId = afterCancelState.currentHeroId;
      const stateWithTreasure: GameState = {
        ...afterCancelState,
        heroes: afterCancelState.heroes.map(h => h.id === heroId ? { ...h, items: ['treasure_healing_potion'] } : h)
      };
      
      useGameStore.setState({ gameState: stateWithTreasure });
      
      const heroBefore = stateWithTreasure.heroes.find(h => h.id === heroId)!;
      const abilitiesCountBefore = heroBefore.abilities.length;
      
      store.discardTreasureForPower(heroId, 'treasure_healing_potion');
      
      const afterDiscardState = useGameStore.getState().gameState;
      if (!afterDiscardState) throw new Error('Adventure 3: afterDiscardState not set');
      
      const heroAfter = afterDiscardState.heroes.find(h => h.id === heroId)!;
      if (heroAfter.items.includes('treasure_healing_potion')) {
        throw new Error('Adventure 3: Treasure card was not discarded');
      }
      if (heroAfter.abilities.length !== abilitiesCountBefore + 1) {
        throw new Error(`Adventure 3: Hero did not gain a new power. Before count: ${abilitiesCountBefore}, After count: ${heroAfter.abilities.length}`);
      }

      // Test Klak Tactic AI - Teleport
      console.log('  Testing Klak AI - Teleport...');
      const stateForAITeleport: GameState = {
        ...afterDiscardState,
        monsters: afterDiscardState.monsters.map(m => {
          if (m.id.startsWith('monster_klak') && !m.id.includes('artifact')) {
            return { ...m, position: { x: 5, z: 5, sqX: 2, sqZ: 2 } };
          }
          return m;
        }),
        tiles: [
          ...afterDiscardState.tiles,
          {
            id: 'far_tile_no_edges',
            name: 'Far Tile',
            x: 5, z: 5,
            terrainType: 'corridor',
            boneSquare: { sqX: 2, sqZ: 2 },
      ...
      const beforeKlakPos = stateForAITeleport.monsters.find(m => m.id.startsWith('monster_klak'))?.position;
      
      const stateAfterAI = await executeVillainPhase(stateForAITeleport);
      const afterKlak = stateAfterAI.monsters.find(m => m.id.startsWith('monster_klak'))!;
      
      if (afterKlak.position.x === beforeKlakPos?.x && afterKlak.position.z === beforeKlakPos?.z) {
        throw new Error('Adventure 3: Klak did not teleport when having no unexplored edges on his tile');
      }
      
      // Verify objectives check
      console.log('  Testing objectives check...');
      const victoryObjectivesState: GameState = {
        ...afterDiscardState,
        monsters: afterDiscardState.monsters.map(m => {
          if (m.id.startsWith('monster_klak') || m.id.startsWith('monster_klaks_artifact')) {
            return { ...m, hp: 0, isDefeated: true };
          }
          return m;
        })
      };
      
      const objectives = ObjectiveTracker.checkObjectives(victoryObjectivesState);
      const klakDefObj = objectives.find(obj => obj.type === 'defeat_klak');
      const artDestObj = objectives.find(obj => obj.type === 'destroy_artifact');
      
      if (!klakDefObj || !klakDefObj.isCompleted) {
        throw new Error('Adventure 3: defeat_klak objective not completed when Klak is dead');
      }
      if (!artDestObj || !artDestObj.isCompleted) {
        throw new Error('Adventure 3: destroy_artifact objective not completed when artifact is dead');
      }
    ...
    console.log('Running Adventure 4: Daylight Assault Verification...');
    {
      ...
      if (!state.timeTrack || state.timeTrack.current !== 1 || state.timeTrack.max !== 6) {
        throw new Error(`Adventure 4: expected timeTrack current=1 max=6, got ${JSON.stringify(state.timeTrack)}`);
      }
      if (state.strahdAwakened !== false) {
        throw new Error(`Adventure 4: expected strahdAwakened to be false initially`);
      }

      // 2. Time Track Advancement on white arrow tile placement
      console.log('  Testing time track advancement on white tile placement...');
      const whiteTileTemplate = DataLoader.getInstance().getTileById('white_x2_01');
      if (!whiteTileTemplate) throw new Error('Adventure 4: white_x2_01 template not found');
      
      const testTile = {
        ...whiteTileTemplate,
        encounterType: 'white' as const
      };
      
      // Force place test tile
      const point = { tileId: 'start-tile', edge: 'north' as const };
      // Override deck to ensure we draw the white tile next
      state.dungeonDeck = [testTile.id];
      useGameStore.setState({ gameState: state });

      const stateAfterPlacement = TileSystem.placeTile(state, point, 0);
      if (!stateAfterPlacement.timeTrack || stateAfterPlacement.timeTrack.current !== 2) {
        throw new Error(`Adventure 4: expected timeTrack current=2, got ${stateAfterPlacement.timeTrack?.current}`);
      }

      // 3. Sunset Check: reach current === max (6)
      console.log('  Testing sunset condition reaching maximum...');
      const stateNearSunset = {
        ...stateAfterPlacement,
        dungeonDeck: ['white_x2_01'],
        timeTrack: { current: 5, max: 6 }
      };
      const stateSunset = TileSystem.placeTile(stateNearSunset, { tileId: 'start-tile', edge: 'south' as const }, 0);
      if (stateSunset.timeTrack?.current !== 6 || stateSunset.strahdAwakened !== true) {
        throw new Error(`Adventure 4: expected timeTrack current=6 and strahdAwakened=true, got current=${stateSunset.timeTrack?.current}, strahdAwakened=${stateSunset.strahdAwakened}`);
      }

      // 4. Sunset Encounter Draw during Villain Phase
      console.log('  Testing sunset Encounter card drawing during Villain Phase...');
      const stateForSunsetVillain: GameState = {
        ...stateSunset,
        cardResolution: { phase: 'idle', cardId: null, cardType: null, targetEntityId: null, pendingEffects: [], resolvedEffects: [], result: null }
      };
      const stateAfterSunsetVillain = await executeVillainPhase(stateForSunsetVillain);
      if (!stateAfterSunsetVillain.cardResolution || stateAfterSunsetVillain.cardResolution.phase !== 'drawing' || stateAfterSunsetVillain.cardResolution.cardType !== 'encounter') {
        throw new Error(`Adventure 4: expected drawing cardResolution for encounter card after sunset, got: ${JSON.stringify(stateAfterSunsetVillain.cardResolution)}`);
      }

      // 5. Escape Mechanic & skip turn progression
      console.log('  Testing escapeHero action and skipped turn order...');
      const stateForEscape: GameState = {
        ...stateSunset,
        heroes: stateSunset.heroes.map(h => 
          h.id === 'hero_arjhan' ? { ...h, position: { x: 0, z: 0, sqX: 1, sqZ: 1 }, items: ['treasure_healing_potion', 'treasure_silver_dagger'] } : h
        ),
        currentHeroId: 'hero_arjhan'
      };
      useGameStore.setState({ gameState: stateForEscape });

      store.escapeHero('hero_arjhan');
      
      let stateAfterEscape = useGameStore.getState().gameState;
      if (!stateAfterEscape) throw new Error('Adventure 4: stateAfterEscape not set');
      
      const escapedHero = stateAfterEscape.heroes.find(h => h.id === 'hero_arjhan')!;
      if (escapedHero.escaped !== true) {
        throw new Error('Adventure 4: escapeHero did not mark hero as escaped');
      }
      
      const startTile = stateAfterEscape.tiles.find(t => t.id === 'start-tile')!;
      if (startTile.heroes.includes('hero_arjhan')) {
        throw new Error('Adventure 4: escaped hero was not removed from start-tile heroes list');
      }

      if (stateAfterEscape.currentHeroId !== 'hero_immeril') {
        throw new Error(`Adventure 4: expected currentHeroId to advance to hero_immeril, got ${stateAfterEscape.currentHeroId}`);
      }

      // 6. Escape count and victory objectives check
      console.log('  Testing escape treasure count and victory...');
      const stateWith12Treasures: GameState = {
        ...stateAfterEscape,
        heroes: stateAfterEscape.heroes.map(h => {
          if (h.id === 'hero_arjhan') {
            return {
              ...h,
              escaped: true,
              items: ['treasure_1', 'treasure_2', 'treasure_3', 'treasure_4', 'treasure_5', 'treasure_6']
            };
          }
          if (h.id === 'hero_immeril') {
            return {
              ...h,
              position: { x: 0, z: 0, sqX: 1, sqZ: 2 },
              items: ['treasure_7', 'treasure_8', 'treasure_9', 'treasure_10', 'treasure_11', 'treasure_12']
            };
          }
          return h;
        }),
        currentHeroId: 'hero_immeril'
      };
      
      useGameStore.setState({ gameState: stateWith12Treasures });
      
      const objectivesChecked = ObjectiveTracker.checkObjectives(stateWith12Treasures);
      const escapeObjective = objectivesChecked.find(obj => obj.type === 'escape_with_treasures')!;
      if (escapeObjective.currentCount !== 6) {
        throw new Error(`Adventure 4: expected current count of treasures to be 6 (only arjhan is escaped), got ${escapeObjective.currentCount}`);
      }

      store.escapeHero('hero_immeril');
      const finalState = useGameStore.getState().gameState;
      if (!finalState) throw new Error('Adventure 4: finalState not set');
      
      const finalObjectives = finalState.activeScenario.objectives;
      const finalEscapeObjective = finalObjectives.find(obj => obj.type === 'escape_with_treasures')!;
      if (finalEscapeObjective.currentCount !== 12 || !finalEscapeObjective.isCompleted) {
        throw new Error(`Adventure 4: expected escape objective to be complete with 12 treasures, got count=${finalEscapeObjective.currentCount}, completed=${finalEscapeObjective.isCompleted}`);
      }
    ...
    console.log('Running Adventure 5: The Final Transformation Verification...');
    {
      ...
      // Confirm power selections to transition to hero phase
      ...
      // Mark power selections as confirmed
      const confirmedSelections = startState.powerSelections?.map(s => ({ ...s, isConfirmed: true })) || [];
      useGameStore.setState({
        gameState: {
          ...startState,
      ...
      const kavanToken = state.tokens?.find(t => t.id === 'token_item_kavan');
      if (!kavanToken) {
        throw new Error('Adventure 5: expected token_item_kavan to be on the board initially');
      }
      if (kavanToken.position.x !== 0 || kavanToken.position.z !== 0) {
        throw new Error(`Adventure 5: expected Kavan to start at (0, 0), got (${kavanToken.position.x}, ${kavanToken.position.z})`);
      }
      if (state.kavanEscortedBy !== 'hero_arjhan') {
        throw new Error(`Adventure 5: expected Kavan to be escorted by hero_arjhan initially, got ${state.kavanEscortedBy}`);
      }

      // 2. Kavan follows hero ending turn
      console.log('  Testing Kavan follows hero ending turn...');
      // Move hero to (1, 0) (e.g. adjacent tile)
      // For testing, let's place a corridor tile at (1, 0)
      const corridorTemplate = DataLoader.getInstance().getTileById('corridor_01') || DataLoader.getInstance().getTileById('white_x2_01');
      if (!corridorTemplate) throw new Error('Adventure 5: corridor template not found');
      const mockTile = {
        ...corridorTemplate,
        id: 'named_chapel_mock', // just some unique ID
        x: 1,
        z: 0
      };
      
      state.tiles = [...state.tiles, mockTile];
      state.heroes = state.heroes.map(h => h.id === 'hero_arjhan' ? { ...h, position: { x: 1, z: 0, sqX: 2, sqZ: 2 } } : h);
      useGameStore.setState({ gameState: state });
      
      store.endTurn(); // This will end the hero turn and check escort movement
      
      let stateAfterTurn = useGameStore.getState().gameState;
      if (!stateAfterTurn) throw new Error('Adventure 5: stateAfterTurn is null');
      const kavanAfterMove = stateAfterTurn.tokens?.find(t => t.id === 'token_item_kavan');
      if (!kavanAfterMove || kavanAfterMove.position.x !== 1 || kavanAfterMove.position.z !== 0) {
        throw new Error(`Adventure 5: expected Kavan to move to (1, 0), got ${JSON.stringify(kavanAfterMove?.position)}`);
      }

      // 3. Transformation when no hero is on Kavan's tile
      console.log('  Testing transformation when no hero is on Kavan\'s tile...');
      // Move hero back to (0, 0), leaving Kavan alone at (1, 0)
      stateAfterTurn.heroes = stateAfterTurn.heroes.map(h => h.id === 'hero_arjhan' ? { ...h, position: { x: 0, z: 0, sqX: 2, sqZ: 2 } } : h);
      useGameStore.setState({ gameState: stateAfterTurn });

      // Run executeVillainPhase to trigger transformation checks
      const stateAfterVillain = await executeVillainPhase(stateAfterTurn);
      const kavanTokenAfterVillain = stateAfterVillain.tokens?.find(t => t.id === 'token_item_kavan');
      if (kavanTokenAfterVillain) {
        throw new Error('Adventure 5: expected Kavan token to be removed after transformation');
      }
      
      const youngVampire = stateAfterVillain.monsters.find(m => m.id.startsWith('monster_young_vampire'));
      if (!youngVampire) {
        throw new Error('Adventure 5: expected Young Vampire to be spawned');
      }
      if (youngVampire.position.x !== 1 || youngVampire.position.z !== 0) {
        throw new Error(`Adventure 5: expected Young Vampire to spawn at (1, 0), got (${youngVampire.position.x}, ${youngVampire.position.z})`);
      }
      if (stateAfterVillain.kavanEscortedBy !== null) {
        throw new Error(`Adventure 5: expected kavanEscortedBy to be null after transformation, got ${stateAfterVillain.kavanEscortedBy}`);
      }

      // 4. Reversion when hero joins him on a clean tile
      console.log('  Testing reversion when hero is on Young Vampire\'s tile with no other monsters...');
      // Move hero to (1, 0) (Young Vampire's tile)
      let stateReversionTest = {
        ...stateAfterVillain,
        heroes: stateAfterVillain.heroes.map(h => h.id === 'hero_arjhan' ? { ...h, position: { x: 1, z: 0, sqX: 2, sqZ: 2 } } : h)
      };
      
      // Let's execute the villain phase queue manually or call executeVillainPhase again
      const stateAfterReversion = await executeVillainPhase(stateReversionTest);
      
      const revertedKavan = stateAfterReversion.tokens?.find(t => t.id === 'token_item_kavan');
      if (!revertedKavan || revertedKavan.position.x !== 1 || revertedKavan.position.z !== 0) {
        throw new Error('Adventure 5: expected Kavan token to be restored at (1, 0) after reversion');
      }
      
      const youngVampireAfterReversion = stateAfterReversion.monsters.find(m => m.id.startsWith('monster_young_vampire'));
      if (youngVampireAfterReversion) {
        throw new Error('Adventure 5: expected Young Vampire monster to be removed after reversion');
      }

      // 5. Decrementing fountain tokens and victory at 0 tokens
      console.log('  Testing cure progression at the Dark Fountain...');
      // Let's place the Dark Fountain tile at (2, 0)
      const fountainTemplate = DataLoader.getInstance().getTileById('named_dark_fountain');
      if (!fountainTemplate) throw new Error('Adventure 5: named_dark_fountain template not found');
      
      const fountainTile = {
        ...fountainTemplate,
        id: 'named_dark_fountain_mock',
        x: 2,
        z: 0
      };
      ...
      let stateCure = {
        ...stateAfterReversion,
        tiles: [...stateAfterReversion.tiles, fountainTile],
        tokens: (stateAfterReversion.tokens || []).map(t => t.id === 'token_item_kavan' ? { ...t, position: { x: 2, z: 0, sqX: 2, sqZ: 2 } } : t),
        heroes: stateAfterReversion.heroes.map(h => h.id === 'hero_arjhan' ? { ...h, position: { x: 2, z: 0, sqX: 2, sqZ: 2 } } : h),
        kavanEscortedBy: 'hero_arjhan' // restore escort
      };
      
      useGameStore.setState({ gameState: stateCure });
      
      // We need to end turn of Hero 5 times to reduce fountainTokens to 0
      for (let i = 0; i < 5; i++) {
        const tempState = useGameStore.getState().gameState!;
        // Make sure hero is on (2, 0) and Kavan is on (2, 0)
        const updatedTempState = {
          ...tempState,
          phase: 'hero' as const,
          currentHeroId: 'hero_arjhan',
          tokens: (tempState.tokens || []).map(t => t.id === 'token_item_kavan' ? { ...t, position: { x: 2, z: 0, sqX: 2, sqZ: 2 } } : t),
          heroes: tempState.heroes.map(h => h.id === 'hero_arjhan' ? { ...h, position: { x: 2, z: 0, sqX: 2, sqZ: 2 } } : h),
          kavanEscortedBy: 'hero_arjhan'
        };
        useGameStore.setState({ gameState: updatedTempState });
        
        store.endTurn();
      }
      
      const finalStateCure = useGameStore.getState().gameState;
      if (!finalStateCure) throw new Error('Adventure 5: finalStateCure is null');
      if (finalStateCure.fountainTokens !== 0) {
        throw new Error(`Adventure 5: expected fountainTokens to be 0, got ${finalStateCure.fountainTokens}`);
      }
      
      // Verify objectives check
      const objectives = ObjectiveTracker.checkObjectives(finalStateCure);
      const cureObj = objectives.find(obj => obj.type === 'cure_kavan');
      if (!cureObj || !cureObj.isCompleted) {
        throw new Error('Adventure 5: cure_kavan objective not completed when fountainTokens reached 0');
      }
      if (finalStateCure.phase !== 'victory') {
        throw new Error(`Adventure 5: expected phase to be victory, got ${finalStateCure.phase}`);
      }

      // 6. Defeat if Young Vampire is defeated
      console.log('  Testing defeat when Young Vampire HP reaches 0...');
      // Start a fresh game to test defeat check
      store.startNewGame('adventure_05', ['hero_arjhan']);
      const defeatState = useGameStore.getState().gameState!;
      
      const confirmedSelections2 = defeatState.powerSelections?.map(s => ({ ...s, isConfirmed: true })) || [];
      useGameStore.setState({
        gameState: {
          ...defeatState,
          powerSelections: confirmedSelections2
        }
      });
      store.beginAdventure();
      
      const startDefeatState = useGameStore.getState().gameState!;
      
      // Spawn Young Vampire
      const template = DataLoader.getInstance().getMonsterById('monster_young_vampire');
      if (!template) throw new Error('Adventure 5: monster_young_vampire template not found');
      
      const deadVampire: Monster = {
        ...template,
      ...
      let stateDeadVampire = {
        ...startDefeatState,
        monsters: [deadVampire]
      };
      
      useGameStore.setState({ gameState: stateDeadVampire });
      
      // Call endTurn or check the defeat transition directly
      store.endTurn();
      
      const finalStateDefeat = useGameStore.getState().gameState;
      if (!finalStateDefeat) throw new Error('Adventure 5: finalStateDefeat is null');
      if (finalStateDefeat.phase !== 'defeat') {
        throw new Error(`Adventure 5: expected phase to be defeat, got ${finalStateDefeat.phase}`);
      }
    ...
    // -----------------------------------------------------------------------
    {
      ...
      // Initialize Adventure 6 with Hero Arjhan
      ...
      // 1. Check deck insertion: Arcane Circle tile (named_arcane_circle) should be placed at index 8 in the deck
      ...
      // 2. Test Arcane Circle tile placement, Gravestorm spawn, Laboratory placement, and Phylactery spawn
      console.log('  Testing Arcane Circle placement and spawn triggers...');
      
      const mockCircleTile: Tile = {
        id: 'named_arcane_circle',
        name: 'Arcane Circle',
        x: 1, z: 0,
        terrainType: 'named_room',
        connections: [
          ...
          { edge: 'east', isOpen: false },
          { edge: 'west', isOpen: false, connectedTileId: 'start-tile' }
        ],
        boneSquare: { sqX: 1, sqZ: 1 },
        isRevealed: false,
        isStart: false,
      ...
      TileSystem.drawAndPlace = () => ({
        tile: mockCircleTile,
        validRotations: [0],
        remainingDeck: state.dungeonDeck.filter(id => id !== 'named_arcane_circle'),
        exhausted: false
      });

      let stateAfterCircle: GameState;
      try {
        stateAfterCircle = TileSystem.placeTile(
          state,
          { tileId: 'start-tile', edge: 'east' },
          0
        );
      ...
      // Check if Gravestorm was spawned
      const gravestorm = stateAfterCircle.monsters.find(m => m.id.startsWith('monster_gravestorm'));
      if (!gravestorm) throw new Error('Adventure 6: Gravestorm was not spawned');
      if (gravestorm.hp !== 20 || gravestorm.ac !== 14 || !gravestorm.isBoss) {
        throw new Error(`Adventure 6: Gravestorm has incorrect stats: HP ${gravestorm.hp}, AC ${gravestorm.ac}, isBoss ${gravestorm.isBoss}`);
      }

      // Check if Laboratory tile is placed
      const labTile = stateAfterCircle.tiles.find(t => t.id.startsWith('named_laboratory'));
      if (!labTile) throw new Error('Adventure 6: Laboratory tile was not automatically placed');

      // Check if Phylactery is spawned on Laboratory
      const phylactery = stateAfterCircle.monsters.find(m => m.id.startsWith('monster_gravestorms_phylactery'));
      if (!phylactery) throw new Error('Adventure 6: Gravestorm\'s Phylactery was not spawned');
      if (phylactery.hp !== 1 || phylactery.ac !== 10 || phylactery.position.x !== labTile.x || phylactery.position.z !== labTile.z || phylactery.position.sqX !== 2 || phylactery.position.sqZ !== 2) {
        throw new Error(`Adventure 6: Phylactery has incorrect stats or position: HP ${phylactery.hp}, AC ${phylactery.ac}`);
      }

      // 3. Test attacking and destroying the Phylactery deals 10 damage to Gravestorm
      console.log('  Testing Phylactery destruction deals 10 damage to Gravestorm...');
      
      useGameStore.setState({ gameState: stateAfterCircle });
      const { CombatSystem } = await import('../game/engine/CombatSystem');
      const originalResolveAttack = CombatSystem.resolveAttack;
      CombatSystem.resolveAttack = (attacker, target) => ({
      ...
      try {
        store.attackMonster(phylactery.id);
      } finally {
        CombatSystem.resolveAttack = originalResolveAttack;
      }

      const stateAfterPhylacteryAttack = useGameStore.getState().gameState!;
      const updatedGravestorm = stateAfterPhylacteryAttack.monsters.find(m => m.id === gravestorm.id)!;
      if (updatedGravestorm.hp !== 10) {
        throw new Error(`Adventure 6: expected Gravestorm HP to be 10 after Phylactery destroyed, got ${updatedGravestorm.hp}`);
      }

      const updatedPhylactery = stateAfterPhylacteryAttack.monsters.find(m => m.id === phylactery.id)!;
      if (updatedPhylactery.hp !== 0 || !updatedPhylactery.isDefeated) {
        throw new Error(`Adventure 6: expected Phylactery HP to be 0 and isDefeated to be true, got HP ${updatedPhylactery.hp}, isDefeated ${updatedPhylactery.isDefeated}`);
      }

      // 4. Test defeating Gravestorm completes the objective and triggers victory
      console.log('  Testing defeating Gravestorm triggers victory...');
      ...
      try {
        store.attackMonster(gravestorm.id);
      } finally {
        CombatSystem.resolveAttack = originalResolveAttack;
      }

      const stateAfterGravestormDefeated = useGameStore.getState().gameState!;
      const deadGravestorm = stateAfterGravestormDefeated.monsters.find(m => m.id === gravestorm.id)!;
      if (deadGravestorm.hp !== 0 || !deadGravestorm.isDefeated) {
        throw new Error(`Adventure 6: expected Gravestorm HP to be 0, got ${deadGravestorm.hp}`);
      }

      if (stateAfterGravestormDefeated.phase !== 'victory') {
        throw new Error(`Adventure 6: expected phase to be victory, got ${stateAfterGravestormDefeated.phase}`);
      }
