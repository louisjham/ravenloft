...
import { getMonsterModelPath, useModel, DUMMY_MODE } from '../../utils/modelLoader';
 ...
 */
const MonsterModel: React.FC<{ monsterId: string }> = ({ monsterId }) => {
  const modelPath = getMonsterModelPath(monsterId);
  const model = useModel(modelPath);
  ...
  const isGravestorm = monsterId.startsWith('monster_gravestorm');
  const scale = isGravestorm ? 1.2 : 0.4;
  // If the dragon is half buried, it means its origin is in the middle of its body instead of its feet.
  // We raise it proportionally to its scale to bring it out of the ground.
  const yPos = isGravestorm ? 1.2 : 0.5;

  return <primitive object={cloned} scale={scale} position={[0, yPos, 0]} />;
};
    ...
    >
      ...
      {DUMMY_MODE ? (
        ...
        <Suspense fallback={<MonsterPlaceholder />}>
          <MonsterModel monsterId={monster.id} />
        </Suspense>
      )}
    </group>
  );
