...
import { Cylinder, Box, Sphere, useGLTF } from '@react-three/drei';
...
const HeroModel: React.FC<{ heroClass: string }> = ({ heroClass }) => {
  const modelPath = getHeroModelPath(heroClass);
  const { scene } = useGLTF(modelPath);
  const model = scene as THREE.Group;
  ...
  return <primitive object={cloned} scale={0.4} position={[0, 0.5, 0]} />;
};
      ...
      <Select enabled={isActive}>
        ...
        {DUMMY_MODE ? <HeroPlaceholder /> : (
          <Suspense fallback={<HeroPlaceholder />}>
            <HeroModel heroClass={hero.heroClass} />
          </Suspense>
        )}
      </Select>
    </group>
